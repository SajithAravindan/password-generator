...
function writePassword() {
  ...
  if (pwdLength != null) {//chk if User presses 'Cancel' option
    if (isNaN(pwdLength) || pwdLength < 8 || pwdLength >= 128) //chk for is a valid number , length > 8 and < 128
    {
      //Give user the option to start again
      if (confirm("Entered length of password is less than 8 OR more than 128 OR is not a valid Number. Do you want to try again?")) {
        writePassword();// loop again
      }
    }
    else {
      if (confirm("Want to Include Uppercase Characters?"))//confirmation to include Uppercase
      {
        userChoices = upperCase;
      }
      if (confirm("Want to Include Lowercase Characters?"))//confirmation to include Lowercase
      {
        userChoices = userChoices + lowerCase;
      }
      if (confirm("Want to Include Numbers?"))//confirmation to include Numbers
      {
        userChoices = userChoices + numbers;
      }
      if (confirm("Want to Include Special Characters?"))//confirmation to include Special Characters
      {
